...
// It took me so much hours do this lab T_T
...
int main(int argc, char *argv[]) {
    ...
    // 2D array where each row is next time step
    ...
    for (int n = 0; n < Nt - 1; ++n) {
        ...
        for (int i = 1; i <= k; ++i) {
            u_local[n + 1][i] = u_local[n][i] - (alpha / 2) * (u_local[n][i + 1] - u_local[n][i - 1]) +
                                (alpha * alpha / 2) * (u_local[n][i + 1] - 2 * u_local[n][i] + u_local[n][i - 1]);
        }
    }

    vector<vector<double>> u_global(Nt, vector<double>(Nx));
